...
# Install terminal fonts on the local machine.
...
if [[ "$OSTYPE" == "darwin"* ]]; then
	...
	# Install Nerd Font if it is missing.
	if ! command -v fc-list &>/dev/null || ! fc-list | grep -q "$fontname Nerd Font"; then
		...
		find "$TEMPDIR/nerd-fonts" -name '* Windows Compatible.ttf' -delete
		mv "$TEMPDIR"/nerd-fonts/*.ttf "$FONTDIR"

		# Install non-Nerd Font Fira Code.
